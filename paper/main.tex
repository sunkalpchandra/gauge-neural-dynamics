...
\IfFileExists{generated/numbers.tex}{\input{generated/numbers}}{}
...
of the maximal torus of $SO(4)$. Three predictions follow: the group the contexts
realise should be abelian, composition should be near exact, and the canonical
latent should have Betti numbers $(1,2,1)$. Since a torus is not preserved by a
general linear map we restrict the gauge to isometries.

All three hold. Composition is essentially exact, GCS \gridGNDgcs, the highest
of the three experiments and what one should expect for the simplest of the three
groups. The realised algebra very nearly commutes, \gridRealisedAbelian{}
against \hipRealisedAbelian{} for the place-cell contexts, whose true group
contains rotations and shears and is genuinely non-abelian (that second figure is
a single-seed value recomputed from the saved generators, since Experiment 1
predates the measure). The distinction
matters: the commutator of the learned \emph{basis} is \gridGNDabelian, which would
read as a failed prediction, but the basis spans six directions while the
contexts exercise a two-dimensional subgroup, and nothing in the objective asks
the unused directions to commute. And the canonical latent is a torus
(Fig.~\ref{fig:grid}c,d) that factorises into two circles carrying the two phases
(Fig.~\ref{fig:grid}e,f), with Betti numbers $(1,2,1)$ recovered in
\gridGNDbetti{} of runs.

Recovery is the one place a linear baseline is hard to beat, and the reason is
worth stating. PCA reaches GRE \gridPCAgre{} against our \gridGNDgre, because
for a single module the leading principal components of the population \emph{are}
the Fourier modes $\cos\theta$ and $\sin\theta$ that constitute the torus
embedding; PCA lands on nearly the right chart without being asked to. What it
does not recover is the group: its GCS is \gridPCAgcs{} against our
\gridGNDgcs, and it finds the toroidal homology in \gridPCAbetti{} of runs
against our \gridGNDbetti. The isometric restriction costs nothing in accuracy
and something in optimisation. On the \gridConvergedSeeds{} seeds where both
variants converge they are indistinguishable on recovery (GRE \gridGNDgreConv{}
against \gridGLgreConv, transport $R^2$ \gridGNDtransportConv{} against
\gridGLtransportConv) while the isometric gauge composes better (GCS
\gridGNDgcs{} against \gridGLgcs); on the remaining \gridFailedSeeds{} it does
not converge, ending at \gridLossRatio{} times the median final training loss.
Table~\ref{tab:grid} reports every seed, which is why its transport $R^2$ carries
a wide interval. A fit is counted as failed on its training loss alone -- above
\gridLossTol{} times the median for that model -- never on the evaluation
...
now the right answer and the realised norm rises from \gridRealisedAbelian{} to
\gridRotRealisedAbelian. What the automorphism costs the linear gauge is
composition rather than chart: GCS falls to \gridRotGcs{} from \gridGNDgcs,
while GRE is \gridRotGre{} and transport $R^2$ \gridRotTransport. Being non-linear
on the $\R^4$ embedding it is a case the flow gauge should handle better, and on
\gridRotFlowBetterSeeds{} seeds it does; on the rest it diverges, leaving it
worse on average (GRE \gridRotFlowGre, transport $R^2$
\gridRotFlowTransport). We report that spread rather than the mean alone,
because the mean of a bimodal outcome describes neither mode. The third family
is a control we expect
...
falls from \gridGNDtransportConv{} to \gridCtrlTransport, on every seed. The
framework says in advance which phenomena lie
...
Appendix~\ref{app:extra}). Two components are indispensable and most of the rest
are not. Removing the gauge, which is exactly the no-context latent model, costs
\ablNoGaugeDelta{} in transport $R^2$ and pushes GRE to \ablNoGaugeGre, past the
no-transformation null. It also attains a perfect GCS of \ablNoGaugeGcs{} while
moving the latent by \ablNoGaugeMag: doing nothing composes flawlessly, which is
the sharpest illustration we have of why GCS must never be read without the
magnitude beside it. Removing the transport term is worse still
(\ablNoTransportDelta), and fails in the opposite direction, learning large
transformations that compose well and mean nothing (magnitude
\ablNoTransportMag, GCS \ablNoTransportGcs, GRE \ablNoTransportGre).

Anchoring is the instructive case. Dropping it costs essentially nothing in fit
(\ablNoAnchorDelta) but raises GRE from \ablFullGre{} to \ablNoAnchorGre, again
past the null. That is exactly what gauge fixing is for: it does not make the
model fit better, it makes the transformation identifiable.

The remaining terms are close to neutral here, and we report that rather than
dropping them. The group-consistency term buys composition consistency (GCS
\ablFullGcs{} with it against \ablNoGroupGcs{} without) at a slight cost in
recovery; the topology term moves neither transport nor MPS appreciably; and the
BCH truncation order makes no measurable difference (GCS \ablBchOneGcs{} at
first order against \ablFullGcs{} at second), because at the transformation
magnitudes reached here the second-order term is small next to the other sources
of error. The latent dimension sweep shows a useful asymmetry: under-parameterising
is costly (\ablDimTwo{} at $d=2$, GRE \ablDimTwoGre) while over-parameterising is
nearly free (\ablDimTwenty{} at $d=20$). Since the true latent dimension of a
recording is unknown, that is the asymmetry one wants.
