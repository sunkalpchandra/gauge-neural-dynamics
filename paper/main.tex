...
The morph context behaves as the geometry says it should
(Table~\ref{tab:hip-per-context}). It is a diffeomorphism but not an affine map,
so the linear gauge cannot express it and scores \hipGNDmorph{} there; the flow
gauge, which extends the same construction to diffeomorphisms, reaches
\hipFlowMorph. Averaged over all contexts the two are closer, since four of the
five are affine and the linear gauge handles those at least as well. The
correspondence-free variant, which matches distributions instead of learning
